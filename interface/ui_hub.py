...
FIXES over v2.4:
  ...
  [FIX 1]  _draw_circuit_panel() uses render_board_only(tmp, override_w, override_h)
           — engine.canvas_w/h are NEVER mutated, wrapped in try/finally as safety net.
  [FIX 2]  CATALOG import moved to module top — not inside a hot 30fps render loop.
  [FIX 3]  Projector window shows active pin connections as coloured wire overlay
...
All other code identical to v2.4.
"""
...
from PyQt6.QtGui  import QImage, QPixmap, QFont, QColor, QPainter, QPen, QKeySequence, QShortcut
from PyQt6.QtCore import Qt, QTimer, pyqtSignal

from interface.ar_overlay     import assemble_final_os_view
from interface.settings_panel import SettingsPanel
# [FIX 2] CATALOG imported once at module level
...
# ─────────────────────────────────────────────────────────────────────────────
#  REUSABLE WIDGETS  (unchanged from v2.4)
# ─────────────────────────────────────────────────────────────────────────────
...
class Sidebar(QFrame):
    ...
    def update_metrics(self, fps: float, state: dict):
        ...
        # [FIX 5] update mode indicator with colour
        mode_colours = {
            'default': C['warn'],
            'draw':    C['accent'],
            'inspect': C['accent3'],
